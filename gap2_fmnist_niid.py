"""Plot selected Fashion-MNIST unseen-client gaps through round 1200.

Selection from completed runs:
alpha=0.5 -> seed 1, alpha=0.8 -> seed 0, alpha=1.0 -> seed 2.

The signed gap is kept unchanged: unseen loss - seen loss.  No absolute
value or non-negative clipping is applied.
"""
...
for alpha_key, seed, label, color in EXPERIMENTS:
    ...
    unseen_loss = load_curve("loss_unseen_clients", alpha_key, seed)
    ax.plot(
        rounds,
        smooth(unseen_loss - seen_loss),
        label=label,
        color=color,
        linestyle="-",
        linewidth=2.0,
...
ax.set_yticks(np.arange(-0.04, 0.081, 0.02))
