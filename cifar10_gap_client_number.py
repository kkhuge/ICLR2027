"""Figure 3: nonnegative CIFAR-10 LeNet gap by participation rate."""
...
def result_paths(clients_per_round):
    stem = (
...
def load_gap(test_path, train_path, smooth=SMOOTH):
    ...
    # Paper convention: clamp negative gaps before applying the moving average.
...
for clients_per_round, color in CURVES:
    test_path, train_path = result_paths(clients_per_round)
...
ax.set_yticks(np.arange(0.0, 0.211, 0.03))
